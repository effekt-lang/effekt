...
lazy val compileBenchmarks = taskKey[List[Benchmark]]("Compiles the benchmark and returns the list of names")
...
lazy val effekt: CrossProject = crossProject(JSPlatform, JVMPlatform).in(file("effekt"))
  ...
  .jvmSettings(
    ...
    collectBenchmarks := {
      ...
      def isEffektFile(file: File): Boolean = file.baseAndExt._2 == "effekt"
      def findArguments(file: File): Benchmark = {
        val base = file.base
        val args = IO.readLines(benchmarksFolder / s"${base}.args")
    ...
    bench := {
      ...
      val benchmarks = compileBenchmarks.value
