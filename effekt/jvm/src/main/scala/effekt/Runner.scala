...
trait Runner[Executable] {
   ...
   */
  def prelude: List[String] = List("effekt", "option", "partial", "stream", "control", "list", "result", "exception", "array", "char", "bytearray", "stringbuffer", "string", "ref")
...
object LLVMRunner extends Runner[String] {
  ...
  /**
   * LLVM's x86_64 shrink-wrapping does not account for the return-address adjustment of guaranteed tail calls:
   * a `tailcc` function with one tail call that must move its return address (the callee takes more stack arguments)
   * and one that need not can leave the stack pointer unbalanced on the second path. Sigh.
   */
  def disableShrinkWrapping(using C: Context): Seq[String] =
    val option = "-enable-shrink-wrap=false"
    // under LTO, code is generated by the linker, and clang does not forward `-mllvm` to it, so we have to do it ourselves...
    val linkerOpts = if (!useLTO) {
      Seq()
    } else {
      System.getProperty("os.name").toLowerCase match {
        case os if os.contains("mac") => Seq(s"-Wl,-mllvm,${option}")
        case os if os.contains("win") => Seq() // shrink-wrapping is already off
        case os                       => Seq(s"-Wl,-plugin-opt=${option}")
      }
    }
    Seq("-mllvm", option) ++ linkerOpts
  ...
  override def build(path: String)(using C: Context): Option[String] =
    ...
    clangArgs ++= disableShrinkWrapping
