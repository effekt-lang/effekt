...
class AAMTests extends munit.FunSuite {
  ...
  test("AAM static parameters: i/acc vary, ks/k are recursion-invariant") {
    val module = parse(loopProgram)
    val static = StaticParameters.analyze(module)
    assertEquals(static(names("loop")), Vector(false, false, true, true))
  }

  test("AAM points-to resolves the recursive call to loop") {
    val module = parse(loopProgram)
    val pointsTo = new PointsTo(module)
  ...
  test("a compositional call reifies and enters its remainder") {
    ...
    """)
    val pointsTo = new PointsTo(module, approximateCallResults = false)
    val application = statements(module).collectFirst {
