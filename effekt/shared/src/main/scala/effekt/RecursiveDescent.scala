  ...
  */
  ...
  def path(): String =
    nonterminal:
      next().kind match {
        case Str(s, false) => s
        case k => fail("path as string literal", k)
      }
  ...
  */
  ...
  inline def backtrack[T](inline p: => T): Maybe[T] =
    ...
    try { Maybe.Some(p, span(before)) } catch {
      case Fail(_, _) => position = before; currentLabel = labelBefore; Maybe.None(span(before))
    }
