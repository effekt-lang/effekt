...
object ArityRaising extends Phase[CoreTransformed, CoreTransformed] {
  ...
  def transform(stmt: Stmt)(using C: Context, DC: DeclarationContext): Stmt = stmt match {
    ...
    case Stmt.Match(scrutinee, clauses, default) =>
      Stmt.Match(transform(scrutinee), clauses.map { case (id, BlockLit(tparams, cparams, vparams, bparams, body)) => (id, BlockLit(tparams, cparams, vparams, bparams, transform(body))) }, default map transform)
    ...
    case Stmt.Shift(id, BlockLit(tparams, cparams, vparams, bparams, body)) =>
      Stmt.Shift(id, BlockLit(tparams, cparams, vparams, bparams, transform(body)))
