      ...
      case CoreTransformed(source, tree, mod, core) => {
        core match {
          case ModuleDecl(path, includes, declarations, externs, definitions, exports) => {
            // Preprocess higher-order block arguments into interfaces
            val newCore = preprocess(core)
            // println(util.show(newCore))
            val declarations = newCore.declarations
            val definitions = newCore.definitions
...
def prependDefns(defns: List[(Id, Block.New)], last: Stmt): Stmt = defns match {
  case head :: rest => Stmt.Def(head._1, head._2, prependDefns(rest, last))
  case Nil => last
}
