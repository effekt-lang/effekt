  ...
  trait Rewrite {
    ...
    def rewrite(e: Extern): Extern = e match {
      ...
      case Extern.Def(id, qualifiedSignature, tps, cps, vps, bps, ret, capts, body) =>
        Extern.Def(rewrite(id), qualifiedSignature, tps map rewrite, cps map rewrite, vps map rewrite, bps map rewrite, rewrite(ret), rewrite(capts), rewrite(body))
    }
  ...
  def substitute(binding: Binding)(using subst: Substitution): Binding =
    binding match {
      ...
      case Binding.ExternApp(id, purity, callee, targs, vargs, bargs) =>
        substitute(callee) match {
          case g: Block.BlockVar =>
            Binding.ExternApp(id, purity, g, targs.map(substitute), vargs.map(substitute), bargs.map(substitute))
          case _ => INTERNAL_ERROR("Should never substitute a concrete block for an FFI function.")
        }
