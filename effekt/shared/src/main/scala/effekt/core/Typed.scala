...
sealed trait Expression extends Term {
  lazy val tpe: ValueType = this match {
    ...
    case pure: Pure => pure.tpe
  }
  // invariant: can only be {} or {io}
  lazy val capt: Captures = this match {
...
enum Pure extends Expression {
  ...
  override lazy val tpe: ValueType = this match {
...
enum Block extends Term {
  ...
  lazy val tpe: BlockType = this match {
...
object BlockType {
  def Function(tparams: List[ValueType.FreeVar], vparams: List[(Name, ValueType)], bparams: List[(Name, BlockType)], result: ValueType): BlockType.Function =
    BlockType.Function(tparams.size,
      vparams.map { case (_, tpe) => close(tpe, tparams, Nil, 0) },
      bparams.map { case (_, tpe) => close(tpe, tparams, Nil, 0) },
      close(result, tparams, bparams.map(_._1), 0))
}

enum ValueType extends Type {
  case FreeVar(name: Name)
  case BoundVar(level: Int, index: Int)
  case Builtin(name: Name, tpeArity: Int)
...
object examples extends scala.App {
  ...
  val TInt = ValueType.Builtin("Int", 0)
  val TBool = ValueType.Builtin("Bool", 0)
  ...
  println(instantiate(polymorphicIdentity, List(TInt), Nil))

  println(instantiate(polymorphicIdentity, List(TInt), Nil) == instantiate(polymorphicIdentity, List(TInt), Nil))
  println(instantiate(polymorphicIdentity, List(TInt), Nil) == instantiate(polymorphicIdentity, List(TBool), Nil))
  ...
  println(instantiate(blockTypePoly, List(TInt), List(Set(Capture.FreeVar("exc")))))
}