...
object Normalizer { normal =>
  ...
  def assertNormal(t: Tree)(using E: ErrorReporter): Unit = Tree.visit(t) {
    ...
    case Stmt.Val(_, binding: (Stmt.Reset | Stmt.Var | Stmt.App | Stmt.Invoke | Stmt.Region | Stmt.Shift | Stmt.Resume), body) =>
     ...
     */
    case t @ Stmt.Val(_, binding, body) =>
    ...
    case t @ Stmt.Match(sc: Make, _, clauses, default) =>
