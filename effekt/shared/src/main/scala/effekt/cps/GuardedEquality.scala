...
object GuardedEquality {
  ...
  private final class Metadata(toplevelParams: List[Id], val body: Stmt) {
    ...
    private def registerSite(
      ...
      if sitesByStmt.get(stmt) == null then {
        ...
        sites += CallSite(stmt, callee, arguments)
      }
    ...
    private def registerDirectTargets(): Unit =
      meta.sites.zipWithIndex.foreach { case (site, index) =>
        definitions.get(site.callee).foreach { info =>
          ...
          if site.arguments.size != info.params.size then {
  ...
  private final class RelativeAnalysis(
    ...
    /** Partition entries by the complete shape containing the observed closure. */
    private def enqueue(function: Id, values: Vector[Origin]): Unit = {
      ...
      val key = SummaryKey(function, guards)
  ...
  private def analyze(toplevelParams: List[Id], body: Stmt): Result = {
    ...
    val facts = meta.definitions.valuesIterator.map { definition =>
      ...
      def lexical(origin: Origin): Origin = origin match {
        case Origin.Symbol(id) => bindings.get(id).fold(origin) {
          case Expr.Variable(other) => lexical(Origin.Symbol(other))
          case expression => Origin.Term(expression)
        }
        case Origin.Closure(function, captures) =>
          Origin.Closure(function, captures.map(lexical))
        case other => other
      }

      val calls = meta.sites.indices.filter { site =>
        targets.targetsAt.getOrElse(site, Set.empty)(definition.id)
      }
      val scope = definition.scope.toSet ++ bindings.keySet
      def relative(origin: Origin): Boolean = origin match {
        case Origin.Symbol(id) => scope(id)
        case Origin.Closure(_, captures) => captures.forall(relative)
        case _ => true
      }
      val direct = calls.nonEmpty && calls.forall { site =>
        val call = meta.sites(site)
        !targets.rigidSites(site) &&
          call.callee == definition.id &&
          call.arguments.size == definition.params.size &&
          call.arguments.forall(relative)
      }

      val entry = if direct then {
        val values = definition.scopeValues.map(lexical)
        val observation = EntryObservation(definition, values, meta.callees.toSet)
        calls.foreach { site =>
          observation.observe(meta.sites(site).arguments.map(lexical))
        }
        observation.result(safe = true)
      } else {
        RelativeAnalysis(meta, targets, definition, bindings).entries()
      }
  ...
  private def recursive(
    ...
    meta.definitions.valuesIterator.filter(d => observed(d.id)).map { definition =>
  ...
  def analyzeRecursion(module: ModuleDecl): RecursiveAnalysis =
    analyzeRecursion(module, module.definitions.iterator.flatMap { definition =>
      targets(definition).localDefinitions.iterator.map(_.id)
    }.toSet)

  def analyzeRecursion(
    module: ModuleDecl,
    observed: Set[Id]
  ): RecursiveAnalysis = {
    ...
    val flows = module.definitions.map { toplevel =>
      ...
      statics ++= recursive(meta, targets, observed)
      targetResult(meta, targets)
    }.toVector
