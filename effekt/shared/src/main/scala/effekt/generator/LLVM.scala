...
object LLVMFragmentPrinter {
  ...
  def wholeProgramOneshot(mainName: BlockSymbol, definitions: List[Definition]): LLVMFragment = {
    implicit val C = LLVMFragmentPrinter.LLVMContext();
    // TODO find proper name of main somehow
    // TODO initialize stack better
...
define void @effektMain() {
    ...
    %env = call i8* @malloc(i64 1024)

    %spp = alloca %Sp
    ; TODO find return type of main
    store %Sp null, %Sp* %spp

    %frm = insertvalue {void (%Sp, %Int)*} undef, void (%Sp, %Int)* @topLevel, 0

    %oldsp = load %Sp, %Sp* %spp
    %oldtypedsp = bitcast %Sp %oldsp to {void (%Sp, %Int)*}*
    %incedtypedsp = getelementptr {void (%Sp, %Int)*}, {void (%Sp, %Int)*}* %oldtypedsp, i64 1
    %incedsp = bitcast {void (%Sp, %Int)*}* %incedtypedsp to %Sp
    %newsp =  call fastcc %Sp @checkOverflow(%Sp %incedsp, %Sp* %spp)
    %newtypedsp = bitcast %Sp %newsp to {void (%Sp, %Int)*}*
    store {void (%Sp, %Int)*} %frm, {void (%Sp, %Int)*}* %newtypedsp
    ; TODO do the store to spp here and not in growStack

    %sp = load %Sp, %Sp* %spp

    tail call fastcc void @${mainName.name}_${mainName.id}(%Env %env, %Sp %sp)
    ret void
}
"""
}
...
// """
  ...
  def asFragment(definition: Definition)(implicit C: LLVMContext): LLVMFragment = definition match {
...
//     ${indentedLines(body.map(asFragment).mkString("\n\n"))}
// }
// """
...
//   }
  ...
  def asFragment(basicBlock: BasicBlock)(implicit C: LLVMContext): LLVMFragment = basicBlock match {
...
//   }
  ...
  def asFragment(operand: Operand)(implicit C: LLVMContext): LLVMFragment = operand match {
    case LocalReference(typ, name) => s"${asFragment(typ)} ${localName(name)}"
    case ConstantNumber(n) => s"i64 $n"
  }

  def asFragment(typ: Type)(implicit C: LLVMContext): LLVMFragment = typ match {
    case VoidType() => "void"
    case I64() => "i64"
    case I8() => "i8"
...
//   }
  ...
  def asFragment(parameter: Parameter)(implicit C: LLVMContext): LLVMFragment = parameter match {
...
//     "{" + types.mkString(", ") + "}"
  ...
  def localName(name: String)(implicit C: LLVMContext): LLVMFragment =
    "%" + name

  def globalName(name: String)(implicit C: LLVMContext): LLVMFragment =
    "@" + name
