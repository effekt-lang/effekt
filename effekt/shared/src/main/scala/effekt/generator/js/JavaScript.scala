...
import effekt.core.optimizer.{DropBindings, Optimizer, TRMC}
...
class JavaScript(additionalFeatureFlags: List[String] = Nil) extends Compiler[String] {
  ...
  lazy val Optimized = allToCore(Core) andThen Aggregate andThen Optimizer andThen TRMC andThen DropBindings map {
