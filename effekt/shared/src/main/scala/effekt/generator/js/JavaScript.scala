...
class JavaScript(additionalFeatureFlags: List[String] = Nil) extends Compiler[String] {
  ...
  override def prettyIR(source: Source, stage: Stage)(using C: Context): Option[Document] = stage match {
    case Stage.Core if C.config.optimize() => Optimized(source).map { (_, _, res) => core.HumanReadablePrettyPrinter.format(res) }
    case Stage.CPS => CPSTransformed(source).map { (_, _, _, res) => cps.PrettyPrinter.format(res) }
    case Stage.Core => Core(source).map { res => core.HumanReadablePrettyPrinter.format(res.core) }
