...
import effekt.core.optimizer.{ Deadcode, DropBindings, Optimizer, TRMC}
...
class JavaScript(additionalFeatureFlags: List[String] = Nil) extends Compiler[String] {
  ...
  lazy val Optimized = allToCore(Core) andThen Aggregate andThen Deadcode andThen core.Show andThen Optimizer andThen TRMC andThen Deadcode andThen DropBindings map {
