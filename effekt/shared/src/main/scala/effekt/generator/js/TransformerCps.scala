...
object TransformerCps extends Transformer {
  ...
  /** JavaScript's value-returning ABI: no payload for zero results, a scalar
   *  for one result, and an array for several results. */
  private def pack(values: List[js.Expr]): js.Expr = values match {
    case Nil => js.Undefined
    case value :: Nil => value
    case values => js.ArrayLiteral(values)
  }

  private def bindNames(results: List[JSName], value: js.Expr): List[js.Stmt] = results match {
    case Nil => List(js.ExprStmt(value))
    case result :: Nil => List(js.Const(result, value))
    case results => List(js.Const(
      js.Pattern.Array(results.map(js.Pattern.Variable.apply)),
      value))
  }

  private def bind(results: List[Id], value: js.Expr): List[js.Stmt] =
    bindNames(results.map(nameDef), value)
  ...
  case class TransformerContext(
    ...
    directCallArguments: Map[cps.Site[cps.Stmt.Call], Map[Int, CallingConvention.FunctionSignature]],
    ...
    directParameters: Map[Id, CallingConvention.FunctionSignature],
    applying: Set[Id],
  ...
  /** Run one CPS computation to its ordinary result vector. */
  private def runCps(callee: js.Expr, arguments: List[js.Expr], results: Int): js.Expr = {
    val ks = freshName("ks_")
    val k = freshName("k_")
    val resultNames = List.fill(results)(freshName("result_"))
    val complete = if results == 1 then js.Variable(k) else
      js.Lambda(
        resultNames,
        js.Return(js.Object(List(JSName("result") ->
          pack(resultNames.map(js.Variable.apply))))))
    val computation = js.Lambda(
      List(ks, k),
      js.Return(js.Call(
        callee,
        arguments ++ List(js.Variable(ks), complete))))
    js.Call(RUN_TOPLEVEL, List(computation))
  }

  /** Adapt a CPS function value to the value-returning ABI. */
  private def toDirectFunction(
    callee: js.Expr,
    signature: CallingConvention.FunctionSignature
  ): js.Expr = {
    val arguments = List.fill(signature.arguments)(freshName("arg_"))
    js.Lambda(
      arguments,
      js.Return(runCps(callee, arguments.map(js.Variable.apply), signature.results)))
  }
  ...
  private def toCpsFunction(
    ...
    val ks = freshName("ks_")
    val k = freshName("k_")
    val results = List.fill(signature.results)(freshName("result_"))
    val call = js.Call(callee, arguments.map(js.Variable.apply))
    js.Lambda(arguments ++ List(ks, k), js.Block(None,
      bindNames(results, call) :+
        js.Return(js.Call(js.Variable(k),
  ...
  private def toDirectFunctionValue(
    value: cps.Expr,
    signature: CallingConvention.FunctionSignature
  )(using ctx: TransformerContext): js.Expr = value match {
    ...
    case cps.Expr.Variable(id) if ctx.directParameters.contains(id) =>
      ...
      require(actual == signature,
        s"Direct parameter $id has signature $actual, expected $signature")
      valueRef(id)
    case _ =>
      toDirectFunction(toValueJS(value), signature)
  }
  ...
  private def directAdapter(id: Id, params: List[Id], worker: JSName)(using ctx: TransformerContext): js.Stmt = {
    ...
    val resume = js.Call(js.Variable(k),
      results.map(js.Variable.apply) :+ js.Variable(ks))
  ...
  private def directImplementation(
    ...
    val parameterSignatures = directParams.zipWithIndex.flatMap { case (param, position) =>
    ...
    val bodyCtx = functionBodyContext.copy(
      ...
      directParameters = ctx.directParameters ++ parameterSignatures,
  ...
  private def directDefinitions(
    ...
    ctx.directWorkers.get(id) match {
      ...
      case None if !ctx.callingConvention.needsCpsEntry(id) => Nil
  ...
  def toJS(
    ...
    lowered match {
      case cps.ModuleDecl(includes, declarations, externs, definitions, _) =>
        ...
        val directCallArguments = targetFlows.iterator.flatMap(_.callTargets).collect {
          case site @ cps.GuardedEquality.CallTargets(
              call @ cps.Stmt.Call(_, _, _: cps.ReturnPoint.Direct), _, _, _, _) =>
            cps.Site(call) -> callingConvention.directArguments(site.targets)
        }.toMap
  ...
  def toJS(e: cps.Expr)(using ctx: TransformerContext): js.Expr = e match {
    ...
    case Expr.Toplevel => js.Variable(JSName("TOPLEVEL_KS"))
  }
   ...
   *
   *  Continuations receive their result vector and a meta-continuation.
   *  Functions whose parameters were dropped simply ignore the extra
   *  arguments.
   */
  ...
  private def directArguments(call: cps.Stmt.Call)(using ctx: TransformerContext): List[js.Expr] = {
    val directPositions = ctx.directCallArguments.getOrElse(cps.Site(call), Map.empty)
  ...
  /** Simultaneously update mutable parameters. */
  private def updateParameters(
    params: List[Id],
    arguments: List[cps.Expr],
    lowered: List[js.Expr]
  )(using ctx: TransformerContext): List[js.Stmt] = {
    val updates = params.zip(arguments).zip(lowered).filterNot {
      case ((param, cps.Expr.Variable(argument)), _) =>
        ctx.renamedCaptures.getOrElse(argument, argument) == param
      case _ => false
    }
    val written = updates.iterator.map(_._1._1).toSet
    val read = updates.iterator.flatMap(_._1._2.free)
      .map(id => ctx.renamedCaptures.getOrElse(id, id)).toSet
    val overlapping = read.intersect(written)
    val assignments =
      if overlapping.isEmpty then
        updates.map { case ((param, _), value) => js.Assign(nameRef(param), value) }
      else {
  ...
  private def jumpTo(
    ...
    updateParameters(join.params, arguments, lowered) :+
  ...
  private def enterJoin(
    ...
    val parameterSignatures = join.params.zipWithIndex.flatMap { case (param, position) =>
      ctx.callingConvention.directParameterSignature(id, position).map(param -> _)
    }.toMap
    val aliases = Option.when(!join.loopified) {
      join.params.zipWithIndex.zip(call.args).collect {
        case ((param, position), cps.Expr.Variable(argument))
            if !parameterSignatures.contains(param) ||
    ...
    val targets = returned.map(resultId => Option.when(rest.free.contains(resultId))(resultId))
  ...
  def toJS(s: cps.Stmt)(using ctx: TransformerContext): Binding[List[js.Stmt]] = s match {
    ...
    case call @ cps.Stmt.Call(cps.Callee.Function(id), args,
          cps.ReturnPoint.Direct(returned, rest))
        if ctx.secondClass.get(id).exists(_.directBody.nonEmpty) =>
      val join = ctx.secondClass(id)
      if ctx.insideBody.contains(id) then
        pure(jumpTo(id, join, args, directArguments(call)))
      else enterJoin(call, id, join, returned, rest)

    case call @ cps.Stmt.Call(callee, args,
          cps.ReturnPoint.Direct(result, rest)) =>
      val arguments = directArguments(call)
      Binding { k =>
        val target = callee match {
          case cps.Callee.Function(id) =>
            toDirectFunctionValue(cps.Expr.Variable(id),
              CallingConvention.FunctionSignature(args.size, result.size))
          case cps.Callee.Method(receiver, method) =>
            js.Member(valueRef(receiver), memberNameRef(method))
        }
        bind(result, js.Call(target, arguments)) ++ toJS(rest).run(k)
      }
    ...
    case call @ cps.Stmt.Call(callee, _,
      ...
      terminalCall(call, callee, call.knownArguments)

    case cps.Stmt.Return(values) => pure(returnValues(values.map(toValueJS)))
  ...
  /** Return to the current lexical destination, independently of how the
   *  result was computed: a join, a CPS entry, or a direct worker. */
  private def returnValues(values: List[js.Expr])(using ctx: TransformerContext): List[js.Stmt] =
    ctx.directResult match {
      case Some((targets, label)) =>
        targets.zip(values).collect {
          case (Some(id), value) => js.Assign(nameRef(id), value)
        } :+ js.Break(Some(label))
      case None => ctx.directContinuation match {
        case Some((ks, k)) =>
          List(js.Return(js.Call(js.Variable(k), values :+ js.Variable(ks))))
        case None if ctx.directBody.nonEmpty =>
          List(if values.isEmpty then js.ReturnVoid else js.Return(pack(values)))
        case None =>
          List(js.Return(js.Object(List(JSName("result") -> pack(values)))))
      }
    }

  /** A residual CPS transfer returns a computation, not a direct result.
   *  Even a region selected for local defunctionalization can retain function
   *  calls. Complete those before returning to the enclosing direct region. */
  private def returnCps(call: js.Expr, bounce: Boolean = false)(using ctx: TransformerContext): List[js.Stmt] =
    ctx.directBody match {
      case Some((owner, _)) =>
        val arity = ctx.callingConvention.resultArity(owner).exact.getOrElse(0)
        val results = List.fill(arity)(freshName("result_"))
        bindNames(results, js.Call(TRAMPOLINE, List(call))) ++
          returnValues(results.map(js.Variable.apply))
      case None =>
        List(js.Return(if bounce then js.Lambda(Nil, call) else call))
    }
  ...
  private def terminalCall(
    ...
    case cps.Callee.Function(id)
        if ctx.directBody.exists(_._1 == id) && ctx.callingConvention.isDirect(id) =>
      val params = ctx.directBody.get._2
      val lowered = args.zipWithIndex.map { case (argument, position) =>
        ctx.callingConvention.directParameterSignature(id, position)
          .fold(toValueJS(argument))(toDirectFunctionValue(argument, _))
      }
      pure(updateParameters(params, args, lowered) :+ js.Continue(Some(nameDef(id))))
      ...
      ctx.defunctionalization.dispatchFor(statement) match {
        ...
        case None => ctx.secondClass.get(id) match {
          case Some(definition) =>
            val lowered = args.zip(definition.params).map { case (argument, parameter) =>
              definition.directParameters.get(parameter) match {
                case Some(signature) => toDirectFunctionValue(argument, signature)
                case None => toArgumentJS(statement, argument)
              }
            }
            val jump = jumpTo(id, definition, args, lowered)
            val last =
              if definition.loopified && ctx.insideBody.contains(id) then
                js.Continue(Some(nameDef(id)))
              else
          ...
          case None =>
            ...
            val call = js.Call(target, lowered)
            pure(returnCps(call,
              bounce = ctx.stackSafety.transferOf(statement) == StackSafety.Transfer.Bounce))
        }
      }
  ...
  private def dispatchLoop(
    ...
    val boundary = Option.when(state.dispatch.boundary) {
      ...
      ) ++ returnCps(call, bounce = true)(using ctx)))
    }
