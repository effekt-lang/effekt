...
/**
 * Converts a bytearray to a null-terminated string, handling zero bytes by encoding them as non-minimal UTF-8 sequences.
 *
...
char* c_bytearray_into_nullterminated_string(const struct Pos arr) {
    uint64_t length = c_bytearray_size(arr);
    uint64_t zero_runes = 0;
    char* data = c_bytearray_data(arr);
