...
define private %Prompt @currentPrompt(%Stack %stack) {
    ...
    %prompt = load %Prompt, ptr %prompt_pointer, !alias.scope !3, !noalias !13
    ret %Prompt %prompt
}
...
define private %Prompt @freshPrompt() {
    ...
    store %PromptValue zeroinitializer, %Prompt %prompt, !alias.scope !4, !noalias !14
    ret %Prompt %prompt
}
...
define private %Object @newObject(%Eraser %eraser, i64 %environmentSize) alwaysinline {
    ...
    store %ReferenceCount 0, ptr %objectReferenceCount, !alias.scope !9, !noalias !12
    store %Eraser %eraser, ptr %objectEraser, !alias.scope !9, !noalias !12
    ret %Object %object
}
...
define private void @shareObject(%Object %object) alwaysinline {
    ...
    next:
    %objectReferenceCount = getelementptr %Header, ptr %object, i64 0, i32 0
    %referenceCount = load %ReferenceCount, ptr %objectReferenceCount, !alias.scope !9, !noalias !12
    %referenceCount.1 = add %ReferenceCount %referenceCount, 1
    store %ReferenceCount %referenceCount.1, ptr %objectReferenceCount, !alias.scope !9, !noalias !12
    br label %done
...
define private void @eraseObject(%Object %object) alwaysinline {
    ...
    free:
    %objectEraser = getelementptr %Header, ptr %object, i64 0, i32 1
    %eraser = load %Eraser, ptr %objectEraser, !alias.scope !9, !noalias !12
...
define private %Stack @getStack(%Prompt %prompt) {
    ...
    %stack = load %Stack, ptr %stack_pointer, !alias.scope !4, !noalias !14
    ret %Stack %stack
}
...
define private %Reference @newReference(%Stack %stack) alwaysinline {
    ...
    %stackPointer = load %StackPointer, ptr %stackPointer_pointer, !alias.scope !3, !noalias !13
...
define private %Stack @checkLimit(%Stack %stack, i64 %n) alwaysinline {
    ...
    %currentStackPointer = load %StackPointer, ptr %stackPointer_pointer, !alias.scope !3, !noalias !13
    %limit = load %Limit, ptr %limit_pointer, !alias.scope !3, !noalias !13
...
realloc:
    ...
    store %StackPointer %newStackPointer, ptr %newStackPointer_pointer, !alias.scope !3, !noalias !13
    store %Limit %newLimit, ptr %newLimit_pointer, !alias.scope !3, !noalias !13
    ...
    store %Stack %newStack, ptr %promptStack, !alias.scope !4, !noalias !14
...
define private %StackPointer @stackAllocate(%Stack %stack, i64 %n) alwaysinline {
    ...
    store %StackPointer %nextStackPointer, ptr %stackPointer_pointer, !alias.scope !3, !noalias !13
...
define private %StackPointer @stackDeallocate(%Stack %stack, i64 %n) {
    ...
    store %StackPointer %newStackPointer, ptr %stackPointer_pointer, !alias.scope !3, !noalias !13
...
define private %Stack @reset(%Stack %oldStack) {
    ...
    store %StackValue %stack.3, %Stack %stack, !alias.scope !3, !noalias !13

    %promptStack = getelementptr %PromptValue, %Prompt %prompt, i64 0, i32 1
    store %Stack %stack, ptr %promptStack, !alias.scope !4, !noalias !14
...
define private void @revalidate(%Stack %stack) {
    ...
    %prompt = load %Prompt, ptr %prompt_pointer, !alias.scope !3, !noalias !13
    %stack_pointer = getelementptr %PromptValue, %Prompt %prompt, i64 0, i32 1
    %promptStack = load %Stack, ptr %stack_pointer, !alias.scope !4, !noalias !14
...
update:
    store %Stack %stack, ptr %stack_pointer, !alias.scope !4, !noalias !14

    %next_pointer = getelementptr %StackValue, %Stack %stack, i64 0, i32 4
    %next = load %Stack, ptr %next_pointer, !alias.scope !3, !noalias !13
...
define private void @invalidate(%Stack %stack, %Stack %end) {
    ...
    %prompt = load %Prompt, ptr %prompt_pointer, !alias.scope !3, !noalias !13
    %stack_pointer = getelementptr %PromptValue, %Prompt %prompt, i64 0, i32 1
    store %Stack null, ptr %stack_pointer, !alias.scope !4, !noalias !14
...
define private %Stack @resume(%Resumption %resumption, %Stack %oldStack) {
    ...
    %start = load %Stack, ptr %rest_pointer, !alias.scope !3, !noalias !13
    call void @revalidate(%Stack %start)

    store %Stack %oldStack, ptr %rest_pointer, !alias.scope !3, !noalias !13
...
define private {%Resumption, %Stack} @shift(%Stack %stack, %Prompt %prompt) {
    ...
    %resumption = load %Stack, ptr %resumpion_pointer, !alias.scope !4, !noalias !14
    %next_pointer = getelementptr %StackValue, %Stack %resumption, i64 0, i32 4
    %next = load %Stack, ptr %next_pointer, !alias.scope !3, !noalias !13

    store %Stack %stack, ptr %next_pointer, !alias.scope !3, !noalias !13
...
define private void @erasePrompt(%Prompt %prompt) alwaysinline {
    ...
    %referenceCount = load %ReferenceCount, ptr %referenceCount_pointer, !alias.scope !4, !noalias !14
...
decrement:
    ...
    store %ReferenceCount %newReferenceCount, ptr %referenceCount_pointer, !alias.scope !4, !noalias !14
    ret void
...
define private %Stack @underflowStack(%Stack %stack) {
    ...
    %prompt = load %Prompt, ptr %stackPrompt, !alias.scope !3, !noalias !13
    %rest = load %Stack, ptr %stackRest, !alias.scope !3, !noalias !13

    %promptStack_pointer = getelementptr %PromptValue, %Prompt %prompt, i64 0, i32 1
    store %Stack null, ptr %promptStack_pointer, !alias.scope !4, !noalias !14
...
define private %Stack @copyStack(%Stack %stack) alwaysinline {
    ...
    store i64 0, ptr %referenceCount_pointer, !alias.scope !3, !noalias !13
...
entry:
    ...
    %referenceCount = load %ReferenceCount, ptr %referenceCount_pointer, !alias.scope !3, !noalias !13
...
copy:
    ...
    store %ReferenceCount %newOldReferenceCount, ptr %referenceCount_pointer, !alias.scope !3, !noalias !13

    %firstCopy = call %Stack @copyStack(%Stack %resumption)

    %stack_pointer = getelementptr %StackValue, %Resumption %resumption, i64 0, i32 4
    %stack = load %Stack, ptr %stack_pointer, !alias.scope !3, !noalias !13
...
loop:
    ...
    store %Stack %newCopy, ptr %rest_pointer, !alias.scope !3, !noalias !13
...
stop:
    store %Stack %firstCopy, ptr %rest_pointer, !alias.scope !3, !noalias !13
    ret %Stack %firstCopy
}
...
define void @shareResumption(%Resumption %resumption) alwaysinline {
    ...
    %referenceCount = load %ReferenceCount, ptr %referenceCount_pointer, !alias.scope !3, !noalias !13
    %referenceCount.1 = add %ReferenceCount %referenceCount, 1
    store %ReferenceCount %referenceCount.1, ptr %referenceCount_pointer, !alias.scope !3, !noalias !13
    ret void
}
...
define void @eraseResumption(%Resumption %resumption) alwaysinline {
    ...
    %stack_pointer = getelementptr %StackValue, %Resumption %resumption, i64 0, i32 4
    %stack = load %Stack, ptr %stack_pointer, !alias.scope !3, !noalias !13
    store %Stack null, ptr %stack_pointer, !alias.scope !3, !noalias !13
...
define void @eraseStack(%Stack %stack) alwaysinline {
    ...
    %stackPointer = load %StackPointer, ptr %stackPointer_pointer, !alias.scope !3, !noalias !13
    %prompt = load %Stack, ptr %prompt_pointer, !alias.scope !3, !noalias !13
    %rest = load %Stack, ptr %rest_pointer, !alias.scope !3, !noalias !13

    %promptStack_pointer = getelementptr %PromptValue, %Prompt %prompt, i64 0, i32 1
    %promptStack = load %Stack, ptr %promptStack_pointer, !alias.scope !4, !noalias !14
...
define private void @shareFrames(%StackPointer %stackPointer) alwaysinline {
    ...
    %sharer = load %Sharer, ptr %stackSharer, !alias.scope !8, !noalias !5
...
define private void @eraseFrames(%StackPointer %stackPointer) alwaysinline {
    ...
    %eraser = load %Eraser, ptr %stackEraser, !alias.scope !8, !noalias !5
...
define private %Stack @withEmptyStack() {
    ...
    %globalsStackPointer = load %StackPointer, ptr %globalsStackPointer_pointer, !alias.scope !3, !noalias !13
    ...
    store ptr @globalsReturn, ptr %returnAddressPointer.0, !alias.scope !8, !noalias !5
    store ptr @topLevelSharer, ptr %sharerPointer.0, !alias.scope !8, !noalias !5
    store ptr @topLevelEraser, ptr %eraserPointer.0, !alias.scope !8, !noalias !5

    %globalsStackPointer_2 = getelementptr %FrameHeader, %StackPointer %globalsStackPointer, i64 1
    store %StackPointer %globalsStackPointer_2, ptr %globalsStackPointer_pointer, !alias.scope !3, !noalias !13

    %stack = call %Stack @reset(%Stack %globals)

    %globalStack = getelementptr %PromptValue, %Prompt @global, i64 0, i32 1
    store %Stack %stack, ptr %globalStack, !alias.scope !4, !noalias !14

    %stackStackPointer = getelementptr %StackValue, %Stack %stack, i64 0, i32 1
    %stackPointer = load %StackPointer, ptr %stackStackPointer, !alias.scope !3, !noalias !13
    ...
    store %ReturnAddress @topLevel, ptr %returnAddressPointer, !alias.scope !8, !noalias !5
    store %Sharer @topLevelSharer, ptr %sharerPointer, !alias.scope !8, !noalias !5
    store %Eraser @topLevelEraser, ptr %eraserPointer, !alias.scope !8, !noalias !5

    %stackPointer_2 = getelementptr %FrameHeader, %StackPointer %stackPointer, i64 1
    store %StackPointer %stackPointer_2, ptr %stackStackPointer, !alias.scope !3, !noalias !13
...
define void @resume_Int(%Stack %stack, %Int %argument) {
    ...
    %returnAddressPointer = getelementptr %FrameHeader, %StackPointer %stackPointer, i64 0, i32 0
    %returnAddress = load %ReturnAddress, ptr %returnAddressPointer, !alias.scope !8, !noalias !5
...
define void @run(%Neg %f) {
    ...
    %functionPointer = load ptr, ptr %functionPointerPointer, !alias.scope !11, !noalias !15
...
!5 = !{!1, !2, !7, !10} ; not stackPointer
!12 = !{!1, !2, !6, !10} ; not object
!13 = !{!2, !6, !7, !10} ; not stackBase
!14 = !{!1, !6, !7, !10} ; not prompt
!15 = !{!1, !2, !6, !7} ; not vtable