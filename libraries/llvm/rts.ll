...
define private %Resumption @newStack(%Prompt %prompt) {

    ; TODO find actual size of stack
    %stack = call ptr @malloc(i64 120)
    %resumption = call ptr @malloc(i64 24)
    ...
    store %StackValue %stack.1, %Stack %stack

    %resumption.0 = insertvalue %ResumptionValue zeroinitializer, %Stack %stack, 1
    %resumption.1 = insertvalue %ResumptionValue %resumption.0, %Stack %stack, 2
...
define %Stack @pushStack(%Resumption %resumption, %Stack %oldStack) {
    %start_pointer = getelementptr %ResumptionValue, %Resumption %resumption, i64 0, i32 1
    %start = load %Stack, ptr %start_pointer
    call void @updatePrompts(%Stack %start); TODO find out if prompts are still valid

    %end_pointer = getelementptr %ResumptionValue, %Resumption %resumption, i64 0, i32 2
...
define private {%Resumption, %Stack} @popStacks(%Stack %stack, %Prompt %prompt) {
    %resumption = call %Resumption @malloc(i64 24)
...
define private %Stack @withEmptyStack() {
    ...
    %resumption = call %Resumption @newStack(%Prompt %prompt)

    %stack = call %Stack @pushStack(%Resumption %resumption, %Stack null)
