...
for i in $(seq 1 $MAX_NESTED_RECORD_SIZE); do
    ...
    python create_nested_record_no_rec.py $i > ../examples/benchmarks/nested_records/nested_records_no_rec_$i.effekt
done
...
for i in $(seq 1 $MAX_LARGE_RECORD_SIZE); do
    ...
    python create_large_record_no_rec.py $i > ../examples/benchmarks/large_records/large_records_no_rec_$i.effekt
done
