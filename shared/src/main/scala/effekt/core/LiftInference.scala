...
//    ModuleDecl(m.path, m.imports, transform(m.defs)(toplevel))
//  }
//
//  // TODO what's with recursive blocks?
//
//  def transform(b: Block)(implicit C: Context): Block = b match {
...
//      }
//      BlockDef(ps, transform(b)(ctx))
//  }
//
//  def transform(s: Stmt)(implicit C: Context): Stmt = s match {
//    case Def(id, block, rest) =>
//      Def(id, transform(block), transform(rest)(Context.bind(id, BlockVar(id))))
...
//      Handle(transform(body)(Context.lift), clauses.map { (s, c) => (s, transform(c)) })
...
//    def current = this
//  }
//  def Context(implicit ctx: Context) = ctx
//
//}