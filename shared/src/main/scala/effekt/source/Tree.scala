 ...
 *
 */
//case class SourceModuleDef(path: String, imports: List[Import], defs: List[Def]) extends Tree
...
case class SourceModuleDef(path: String, imports: List[Import], defs: List[Def]) extends ModuleDef
case class LocalModuleDef(id: IdDef, defs: List[Def]) extends ModuleDef with Def {
  type symbol = symbols.LocalModule
}
